...
client.on("guildMemberAdd", member => {
  member.roles.add('786689371990261792'); // UNREGİSTER ROLÜNÜN İDSİNİ GİRİN
});
...
client.on("guildMemberAdd", member => {  
    const kanal = member.guild.channels.cache.find(r => r.id === "787072512466747412");
    const register = "<@&786688330879598643>"
...
client.on("userUpdate", async (oldUser, newUser) => {
  ...
  const tag = 'ৡ'
  const sunucu = '767506171011137566'
  const kanal = '789067194033242142'
  const rol = '786690023751680030'
...
client.on("userUpdate", async (stg, yeni) => {
  var sunucu = client.guilds.cache.get('767506171011137566'); // Buraya Sunucu ID
  var uye = sunucu.members.cache.get(yeni.id);
  var tag = "ৡ"; // Buraya Ekip Tag
  var tagrol = "786690023751680030"; // Buraya Ekip Rolünün ID
  var kanal = "789067194033242142"; // Loglanacağı Kanalın ID
